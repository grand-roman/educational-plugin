...
abstract class EduTestCase : LightPlatformCodeInsightFixtureTestCase() {
  ...
  // TODO: come up with better name when we will set up not only virtual file listeners
  protected inline fun withVirtualFileListener(course: Course, action: () -> Unit) {
    val virtualFileManager = VirtualFileManager.getInstance()

    val listener = if (course.isStudy) EduVirtualFileListener(project) else CCVirtualFileListener(project)
